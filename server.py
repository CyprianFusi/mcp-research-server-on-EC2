...
This server allows saving, searching, listing, deleting, and retrieving information
about research data organized by topics using ChromaDB and OpenAI embeddings.
"""
...
def save_research_data(content: List[str], topic: str = "default") -> str:
    ...
    try:
        ...
        if not new_content:
            return f"No new content to save - all {len(content)} documents already exist in topic: {topic.replace('_', ' ').title()}"
...
def search_research_data(query: str, topic: str = "default", max_results: int = 5) -> str:
    ...
    try:
        ...
        if not topic_path.exists():
            return f"No research data found for topic: {topic.replace('_', ' ').title()}"
        ...
        try:
            ...
            if count == 0:
                return f"No documents found in topic: {topic.replace('_', ' ').title()}"
        except:
            return f"No research data found for topic: {topic.replace('_', ' ').title()}"
        ...
        if not results:
            return f"No relevant results found for query: '{query}' in topic: {topic.replace('_', ' ').title()}"
...
def delete_research_topic(topic: str) -> str:
    ...
    try:
        ...
        if not topic_path.exists():
            return f"Topic '{topic.replace('_', ' ').title()}' does not exist"
        ...
        return f"Successfully deleted topic: {topic.replace('_', ' ').title()}"
